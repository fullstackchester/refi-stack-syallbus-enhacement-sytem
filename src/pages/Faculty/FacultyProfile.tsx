...
export default function FacultyProfile() {
    ...
    const [currType, setCurrType] = useState<UserRole | ''>('')
    // references
    ...
    useEffect(() => {
        onValue(ref(database, `users/${uid}`), snapshot => {
            if (snapshot.exists()) {
                ...
                setCurrType(profile.userType)

            }
        })
    }, [])

    function EditFaculty(e: React.FormEvent<HTMLFormElement>) {
        e.preventDefault()
        if (!idRef.current || !nameRef.current || !deptRef.current || !typeRef.current) {
            return
        }
        setLoading(true)
    ...
    const fields = [
        ...
        },
        {
            ...
            description: '',
            options: department,
            onChange: (e: React.ChangeEvent<HTMLSelectElement>) => setCurrDept(e.target.value)
        },

        {
            ...
            options: accType,
            onChange: (e: React.ChangeEvent<HTMLSelectElement>) => setCurrType(e.target.value as UserRole)
        },
    ]
    return (
        <>
                            ...
                            <label
                                ...
                                {val.type !== 'select' ?
                                    <input
                                        name={val.name}
                                        id={val.id}
                                        ref={val.id === 'employee-id' ? idRef : nameRef}
                                    ...
                                    <select
                                        name={val.name}
                                        id={val.id}
                                        ref={val.id === 'dept' ? deptRef : typeRef}
                                        disabled={val.disabled}
                                        value={val.value}
