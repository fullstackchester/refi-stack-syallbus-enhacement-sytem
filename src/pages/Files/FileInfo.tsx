...
export default function FileInfo() {
    ...
    useEffect(() => {
        return onValue(ref(database, `/posts/${id}`), snapshot => {
            if (snapshot.exists()) {
                const currentPost = snapshotValue<Post>(snapshot)
                setPost(currentPost)
                onValue(ref(database, `subject/${currentPost.subjectId}`), subjectSnapshot => {
                    if (subjectSnapshot.exists()) {
                        setSubject(snapshotValue<Subject>(subjectSnapshot).subjectTitle)
                    } else {
                        setSubject('Subject not found')
                    }
                })
                getDownloadURL(storageRef(storage, currentPost.postFileUrl ?? ''))
                    .then((url) => {
                        setFileUrl(url)
                    })
                    .catch((e) => {
    ...
    function DeletePost(e: MouseEvent<HTMLButtonElement>) {
        e.preventDefault()

        if (!post?.postFileUrl) return
