...
function Dashboard() {
    const [AY, setAY] = useState<SchoolYear[]>([])
    ...
    const syStartRef = useRef<HTMLInputElement>(null)
    const syEndRef = useRef<HTMLInputElement>(null)
    ...
    function addSy(e: React.FormEvent<HTMLFormElement>) {
        e.preventDefault()
        if (!syStartRef.current || !syEndRef.current) return
        const id = uuidv4()
        const newSy = {
            syId: id,
    ...
    function deleteSy(sy: SchoolYear) {
                            ...
                            [
                                ...
                                <Input
                                    ...
                                    <input
                                        name={v.name}
                                        id={v.id}
                                        ref={v.id === 'sy-start' ? syStartRef : syEndRef}
                                        type={v.type}
                                        required={v.require}
