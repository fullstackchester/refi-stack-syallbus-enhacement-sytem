...
function SubjectEdit() {
    ...
    const creditUnitRef = useRef<HTMLInputElement>(null)
    ...
    useEffect(() => {
        const getCurrentData = onValue(ref(database, 'subject/' + id), snapshot => {
            if (snapshot.exists()) {
                setSubject(snapshotValue<Subject>(snapshot))
            } else {
    ...
    function UpdateSubject(e: FormEvent<HTMLFormElement>) {
        e.preventDefault()
        setLoading(true)
        setOpen(true)
                ...
                <div className='h-[90vh] w-[85%] bg-white rounded-md flex flex-col'>
                    ...
                    <main className='flex-1 py-2 px-3 flex flex-col'>
                        <form
                            ...
                            <Input htmlFor='course-description' width='col-span-4' label='Course description'>
                                <textarea
